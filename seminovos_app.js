...
createApp({
    ...
    data() {
        return {
            ...
            carros: [
                ...
                {
                  id: 8, // ID corrigido para não repetir
                    nome: 'Fiat Pulse Drive 1.3 AT (Prata)',
                    ano: 2024,
                    quilometragem: '4.000',
                    cor: 'Prata',
                    preco: 'R$ 110.850,00',
    ...
    computed: {
        carrosFiltrados() {
            ...
            // PREPARAÇÃO: Remove acentos e coloca tudo em minúsculo para comparar
            const termoLimpo = this.termoBusca.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, "");
            // Divide o que foi digitado em palavras separadas (ex: "fiat drive" vira ["fiat", "drive"])
            ...
            return this.carros.filter(carro => {
                ...
                // LÓGICA DE PRECISÃO: Verifica se TODAS as palavras digitadas estão no nome do carro
                // Isso permite achar "Fiat Argo Drive" digitando apenas "argo drive" ou "fiat drive"
    ...
    // Garante que a animação funcione quando a lista é filtrada
    updated() {
